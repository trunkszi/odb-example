...
namespace odb
{
  namespace mssql
  {
      ...
      case chunk_next:
        {
          ...
          // fit into our buffer. There could also be more model but since
          // there is no way to stop until we read all the model, dump
                     ...
                     std::size_t tmp_capacity)
    {
      ...
      case chunk_last:
        {
          // Append the model from the temporary buffer.
          //
          if (*pos < N)
          {
      ...
      case chunk_next:
        {
          ...
          // no way to stop until we read all the model, so dump the
