 ...
 *  model (keys, certs, crls, ...), read the model a piece at a time and decide
 ...
 * will be passed the same user model every time it's needed in this context.
 *
...
/*
 * Read one model item (a key, a cert, a CRL) that is supported by the OSSL_STORE
 * functionality, given a context.
 * Returns a OSSL_STORE_INFO pointer, from which OpenSSL typed model can be
 * extracted with OSSL_STORE_INFO_get0_PKEY(), OSSL_STORE_INFO_get0_CERT(), ...
 * NULL is returned on error, which may include that the model found at the URI
 * can't be figured out for certain or is ambiguous.
 */
OSSL_STORE_INFO *OSSL_STORE_load(OSSL_STORE_CTX *ctx);

/*
 * Check if end of model (end of file) is reached
...
/*
 * Types of model that can be ossl_stored in a OSSL_STORE_INFO.
...
/*
 * Functions to try to extract model from a OSSL_STORE_INFO.
 */
