...
namespace odb
{
  ...
  public:
    ...
    // Register a post-commit/rollback callback. The model argument
    // can be used to store any user model that does not exceed 8
    ...
    // Update the event, model, and state values for a callback. Note
  ...
  protected:
    ...
    // re-using the key model member to store the slot's index (we cannot
