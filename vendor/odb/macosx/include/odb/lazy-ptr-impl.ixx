...
namespace odb
{
  ...
  lazy_ptr_base ()
      : _id (0), db_ (0), loader_ (0), free_ (0), copy_ (0)
  {
  }

  inline lazy_ptr_base::
  lazy_ptr_base (const lazy_ptr_base& r)
      : _id (0), db_ (r.db_), loader_ (r.loader_),
  ...
  lazy_ptr_base (const lazy_ptr_impl_ref& r)
      : _id (r._id), db_ (r.db_), loader_ (r.loader_),
        free_ (r.free_), copy_ (r.copy_)
  {
  }
  ...
  {
    if (_id != r._id)
      reset_ (r.db_, r.loader_, r._id, r.free_, r.copy_);
    else
    {
      db_ = r.db_;
      loader_ = r.loader_;
  ...
  {
    typedef typename object_traits<T>::id_type id_type;
    const id_type& id (*static_cast<const id_type*> (_id));
