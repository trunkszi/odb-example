...
namespace odb
{
  namespace mssql
  {
    ...
    struct query_param_impl<T, id_string>: query_param
    {
      query_param_impl (ref_bind<T> r)
          : query_param (r.ptr ()),
            // Default to short model max (1024).
    ...
    struct query_param_impl<T, id_nstring>: query_param
    {
      query_param_impl (ref_bind<T> r)
          : query_param (r.ptr ()),
            // Precision is in 2-byte chars. Default to short model max (1024).
    ...
    //
    // For long model we cannot assume that the by-value parameter will
    // still be alive when we execute the query (and when the callback
    // will be called to provide the model). So we have to make a private
    // copy of the model and use that when the time comes.
    //
                                                ...
                                                long_query_param_impl
    {
      ...
      bind (bind_type* b)
      {
        ...
        // buffer containing all the model. So instead of using the
        // callback mechanism, bind the buffer directly using the
        // short model approach. SQLLEN  (int on 32-bit platforms)
