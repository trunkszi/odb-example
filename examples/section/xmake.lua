...
target("section", function()
    on_load(function(target)
        os.cd("$(projectdir)/examples/section")
        if is_plat("windows") then
            -- windows平台需要安装build2构建系统
            os.setenv("PATH", os.getenv("PATH") .. ";" .. path.translate("C:/build2/bin"))
            os.execv(path.absolute("../../vendor/odb/windows/bin/odb.exe"), { "--std", "c++14", "-d", "pgsql", "--generate-query", "--generate-schema", "person.h" })
        elseif is_plat("linux") then
            ...
            os.execv(path.absolute("../../vendor/odb/linux/bin/odb"), { "--std", "c++14", "-d", "pgsql", "-I", path.absolute("../../vendor/odb/linux/include"), "--generate-query", "--generate-schema", "person.h" })
        elseif is_plat("macosx") then
            ...
            os.execv(path.absolute("../../vendor/odb/macosx/bin/odb"), { "--std", "c++14", "-d", "pgsql", "-I", path.absolute("../../vendor/odb/macosx/include"), "--generate-query", "--generate-schema", "person.h" })
        end
