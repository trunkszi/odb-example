...
static void query_factory(const char *name, connection &c) {
...
static void print_ages(unsigned short age, odb::result<person> r) {
  cout << "over " << age << ':';
...
int main(int argc, char *argv[]) {
  try {
    ...
    for (unsigned short i(0); i < 5; ++i) {
      ...
      if (!pq) {
        pq = db->prepare_query<person>("person-val-age-query", query::age > 50);
        db->cache_query(pq);
      }
    ...
    for (unsigned short age(90); age > 40; age -= 10) {
      ...
      if (!pq) {
        ...
        db->cache_query(pq, std::move(p)); // Assumes ownership of p.
      }
    ...
    */
    ...
    //
    {
      typedef odb::query<person_count> query_count;
      typedef odb::prepared_query<person_count> prep_query_count;

      transaction t(db->begin());

      unsigned short age;
      query_count q(query::age > query::_ref(age));
      prep_query_count pq(
