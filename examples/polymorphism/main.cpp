...
main (int argc, char* argv[])
{
  try
  {
    ...
    //
    {
      ...
      shared_ptr<person> p2 (new contractor ("Jane", "Doe", "jane@doe.com"));
    ...
    {
      transaction t (db->begin ());
      shared_ptr<person> p1 (db->load<person> (id1)); // Loads employee.
      shared_ptr<person> p2 (db->load<person> (id2)); // Loads contractor.
      t.commit ();
    ...
    //
    {
      ...
      shared_ptr<employee> e (db->load<employee> (id1));
      e->temporary (false);
      person& p (*e);
    ...
    {
      transaction t (db->begin ());
      shared_ptr<person> p (db->load<person> (id1)); // Loads employee.
