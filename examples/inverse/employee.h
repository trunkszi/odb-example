...
public:
  ...
  employer(const employer &emp) {
    ...
    this->employees_ = emp.employees_;
  }
  ...
  const employees_type &employees() const { return employees_; }

  employees_type &employees() { return employees_; }
...
class position {
public:
  position(const std::string &title) : title_(title) {}
  ...
  const lazy_weak_ptr<employee_type> &employee() const { return employee_; }

  void employee(lazy_weak_ptr<employee_type> employee) { employee_ = employee; }
...
public:
  ...
  employee(const std::string &first, const std::string &last,
      ...
      : first_(first), last_(last), employer_(employer), position_(position) {}
  ...
  // Employer.
  //
  const lazy_shared_ptr<employer_type> &employer() const { return employer_; }

  void employer(lazy_shared_ptr<employer_type> employer) {
    employer_ = employer;
  }

  // Position.
  //
  const lazy_shared_ptr<position_type> &position() const { return position_; }

  void position(lazy_shared_ptr<position_type> position) {
    position_ = position;
  }
  ...
  const projects_type &projects() const { return projects_; }

  projects_type &projects() { return projects_; }
